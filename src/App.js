...
const Row = (props) => {
  ...
  return (
    <div className='row'>
      {[...new Array(5)].map((x, i) => {
        return (
          <Cell key={i} handleChange={handleChange} isMatch={matches[i]} didWin={props.win} row={props.row} cellID={i} counter={props.counter} cellCounter={cellCounter} /> // not using counter or cellCounter in the props as of now
        )
      })}
    </div>
  );
